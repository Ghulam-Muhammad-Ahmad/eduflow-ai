...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const completion = await chatComplete({
      prompt: buildMatchingPrompt(requirement as unknown as Record<string, unknown>, tutorsForAI),
      system: "You are an expert tutor matching advisor. Return only valid JSON, no markdown.",
      taskType: "tutor_matching",
      temperature: 0.3,
      json: true,
    });
    ...
    return res.status(500).json({
      error: error instanceof Error ? error.message : "Failed to match tutors",
    });
  }

  let aiResult: Record<string, unknown>;
  try {
    aiResult = parseJsonResponse<Record<string, unknown>>(raw);
  } catch {
