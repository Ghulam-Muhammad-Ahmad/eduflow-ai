...
const TeacherStudentRecordPage = () => {
  ...
  const { studentId, classroomId } = router.query;
  ...
  const { data, isLoading, error } = useTeacherStudentRecords(classroomIdStr ?? null);
  const students = data?.students ?? [];
  const student = studentIdStr
    ? students.find((s) => s.student_id === studentIdStr && s.classroom_id === (classroomIdStr ?? ""))
    : null;
  ...
  if (isLoading || !studentIdStr || !classroomIdStr) {
    return (
      <DashboardLayout>
  ...
  if (!student) {
    return (
      <DashboardLayout>
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground">Student record not found. They may not be in this classroom.</p>
            <Button variant="outline" className="mt-4" onClick={handleBack}>
              <ArrowLeft className="w-4 h-4 mr-2" />
              Back to Student Records
            </Button>
